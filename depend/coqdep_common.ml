    ...
    let loop_body acc =
      ...
      | Require strl ->
          ...
          begin
            ...
            with Not_found ->
		          if verbose && not (Hashtbl.mem coqlibKnown str)
              then warning_module_notfound f str; acc
          end
          else acc) acc strl
	      | RequireString s ->
          let str = Filename.basename s in
          ...
          begin
            ...
            try
              ...
              (canonize file_str, !suffixe) :: acc
            with Not_found ->
	           (if not (Hashtbl.mem coqlibKnown [str])
             then warning_notfound f s); acc
          end
          else acc
	      | Declare sl ->
	        let declare suff dir s =
          ...
          in
            ...
            begin
              ...
              | None ->
                ...
                | None -> warning_declare f str; acc
            end
            else acc
	        in List.fold_left decl acc sl
        | Load str ->
	        let str = Filename.basename str in
	        if not (List.mem [str] !deja_vu_v)
          then
